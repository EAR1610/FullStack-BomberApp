...
const handleRoleChange = (e:any) => {
  ...
  const handleSubmit = async ( e:React.FormEvent<HTMLFormElement> ) => {    
    ...
    try {
      if (data) {
        const res = await apiRequestAuth.put(`/${data.id}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${currentToken?.token}`
          },
        });
        toast.current.show({ severity: 'info', summary: 'Info', detail: 'Usuario Actualizado!' });
      } else {
        ...
        toast.current.show({ severity: 'info', summary: 'Info', detail: 'Usuario Creado!' });
      }
